...
<div class="fieldcontain ${hasErrors(bean: resourceInstance, field: 'resourceHasUniverses', 'error')} ">
	<label for="resourceHasUniverses">
		<g:message code="resource.resourceHasUniverses.label" default="Resource Has Universes" />
		
	</label>
	
<ul class="one-to-many">
<g:each in="${resourceInstance?.resourceHasUniverses?}" var="r">
    <li><g:link controller="resourceHasUnivers" action="show" id="${r.id}">${r?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="resourceHasUnivers" action="create" params="['resource.id': resourceInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'resourceHasUnivers.label', default: 'ResourceHasUnivers')])}</g:link>
</li>
</ul>
...
<g:each in="${resourceInstance?.extTags?}" var="r">
    <li><g:link controller="resourceHasResourceTag" action="show" id="${r.id}">${r?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="resourceHasResourceTag" action="create" params="['resource.id': resourceInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'resourceHasResourceTag.label', default: 'ResourceHasResourceTag')])}</g:link>
</li>
</ul>
