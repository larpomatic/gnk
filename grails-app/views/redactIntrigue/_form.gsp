...
<li class="add">
<g:link controller="event" action="create" params="['plot.id': plotInstance?.id]">{message(code: 'default.add.label', args: [message(code: 'event.label', default: 'Event')])}</g:link>
</li>
</ul>
...
<div class="tabbable">
	...
	<g:formRemote name="myForm" update="updateMe" url="[controller: 'role', action:'save']"></g:formRemote><!-- the magic thing which makes it work -->
					<g:formRemote name="myForm"	update="updateMe" url="[controller: 'role', action:'save']" onSuccess="alert('Rôle correctement mis à jour !')" onFailing="alert('Erreur !')">
								...
								<table>
									<g:each in="${roleTagList}" status="i" var="roleTagInstance">
										...
										<td><label for="roleTags_${roleTagInstance.id}"><g:checkBox
													...
													id="roleTags_${roleTagInstance.id}" checked="false" /> ${fieldValue(bean: roleTagInstance, field: "tag")}</label></td>
										<g:if test="${(i+1) % 3 == 0}">
											</tr>
										</g:if>
									</g:each>
...
<g:formRemote name="myForm"
									update="updateMe" url="[controller: 'role', action:'save']"  onSuccess="alert('Rôle correctement mis à jour !')" onFailing="alert('Erreur !')">
									<input type="hidden" name="plotId" value="${plotInstance?.id }"/>
									<table>
										<tr>
											<td><label for="roleCode"> <g:message
		...
		<table>
			<g:each in="${plotTagList}" status="i" var="plotTagInstance">
				...
				<td><label for="tags_${plotTagInstance.id}"><g:checkBox
							...
							checked="${plotInstance.hasPlotTag(plotTagInstance)}" /> ${fieldValue(bean: plotTagInstance, field: "tag")}</label></td>
				<g:if test="${(i+1) % 3 == 0}">
					</tr>
				</g:if>
			</g:each>
