...
*/
    ...
    def sortGenericResourceObjectTypeList(ArrayList<GenericResource> PList) {
        ...
        for (Integer i = 0; i <= 3; i++) {
            ...
            for (String n : nameList)
                for (GenericResource gr : PList)
                    if (null != gr.selectedResource && n == gr.selectedResource.name) {
                        tmpList.add(gr)
                        PList.remove(gr)
                        break
                    }
            ...
            ArrayList<RoleHasPastscene> tmp = new ArrayList<RoleHasPastscene>();
                ...
                for (Role r : roles) {
                    for (RoleHasPastscene roleHasPastscene : r.roleHasPastscenes) {
                        ...
                        Integer time = roleHasPastscene.pastscene.relativeTime
                        String unit = roleHasPastscene.pastscene.relativeTimeUnit
